...
// @version        1.5.3
// @description    Divides the lesson progress bar into milestones with progressive rarities + particle burst + EN/ES settings panel.
...
const I18N = {
  es: {
    panelTitle:      '⚙ Hitos de barra (ADHC)',
    langLabel:       'Idioma',
  ...
  en: {
    panelTitle:      '⚙ Progress bar milestones (ADHC)',
    langLabel:       'Language',
...
if (typeof window !== 'undefined' && typeof document !== 'undefined') {
  (function () {
    const stored = GM_getValue('adhc_config', {});
    const cfg = Object.assign({}, DEFAULTS, stored, { lang: stored.lang || detectLang(navigator.language) });
    function save() { GM_setValue('adhc_config', cfg); }

    // ---------- Adaptación de tema claro/oscuro (Duolingo dark mode) ----------
    // Detecta por la luminancia del fondo del body: dark si < 128.
    GM_addStyle(`
      body.adhc-dark .adhc-btn { background:#1f2b33; color:#1cb0f6; box-shadow:0 4px 0 #0c1419; }
      body.adhc-dark .adhc-btn:active { box-shadow:0 0 0 #0c1419; }
      body.adhc-dark .adhc-panel { background:#1e2a31; box-shadow:0 4px 16px rgba(0,0,0,.55); }
      body.adhc-dark .adhc-panel label { color:#c9d1d9; }
      body.adhc-dark .adhc-panel input[type=range], body.adhc-dark .adhc-panel select { background:#142026; border-color:#3a4a52; color:#e5e5e5; }
      body.adhc-dark .adhc-panel h4 { color:#fff; }
      body.adhc-dark .adhc-divider { border-top-color:#3a4a52; }
      body.adhc-dark .adhc-panel button { background:#142026; border-color:#3a4a52; color:#e5e5e5; box-shadow:0 4px 0 #0b1216; }
      body.adhc-dark .adhc-panel button:active { box-shadow:0 0 0 #0b1216; }
      body.adhc-dark .adhc-sep-num { background:#1f2b30; }
    `);
    ...
    function applyTheme() {
      document.body.classList.toggle('adhc-dark', isDarkTheme());
    }
    applyTheme();
    ...
    GM_addStyle(`
      ...
      .adhc-overlay { position:absolute; inset:0; overflow:hidden; pointer-events:none; z-index:3; border-radius:calc(var(--__internal__progress-bar-height, 16px) / 2); }

      /* tramo base: píldora + shine (réplica de ._27NV6 / ._1EFTr nativos) */
      .adhc-seg { position:absolute; top:0; bottom:0; overflow:hidden; transition:width .4s ease; will-change:width; }
      .adhc-seg:first-child { border-radius:calc(var(--__internal__progress-bar-height, 16px) / 2) 0 0 calc(var(--__internal__progress-bar-height, 16px) / 2); }
      .adhc-seg:last-child  { border-radius:0 calc(var(--__internal__progress-bar-height, 16px) / 2) calc(var(--__internal__progress-bar-height, 16px) / 2) 0; }
      .adhc-seg:only-child  { border-radius:calc(var(--__internal__progress-bar-height, 16px) / 2); }
      .adhc-seg::after { content:''; position:absolute; left:4%; right:4%; top:25%; height:30%; background:#fff; opacity:.2; border-radius:9999px; pointer-events:none; }

      /* ===== TRAMO LEGENDARIO (el final): gradiente épico animado + glow + sparkles ===== */
      .adhc-seg.adhc-legendary {
        background: linear-gradient(135deg,#4c1d95,#7c3aed,#d946ef,#f59e0b,#fbbf24,#d946ef,#4c1d95);
        background-size: 400% 400%;
        animation: adhc-legend-bg 2.5s ease infinite, adhc-legend-glow 1.2s ease-in-out infinite;
        box-shadow: 0 0 14px rgba(168,85,247,.55), inset 0 0 8px rgba(255,255,255,.35);
      }
      @keyframes adhc-legend-bg { 0%,100%{background-position:0% 50%} 50%{background-position:100% 50%} }
      @keyframes adhc-legend-glow { 0%,100%{box-shadow:0 0 8px rgba(168,85,247,.4), inset 0 0 6px rgba(255,255,255,.3)} 50%{box-shadow:0 0 20px rgba(216,180,254,.9), inset 0 0 10px rgba(255,255,255,.5)} }
      /* ===== shines animadas (lenguaje nativo: franjas redondeadas como ._1EFTr) ===== */
      .adhc-seg.adhc-legendary::before, .adhc-seg.adhc-legendary::after {
        content:''; position:absolute; border-radius:9999px; background:#fff;
        animation: adhc-shine-pulse var(--dur,3.2s) ease-in-out infinite;
      }
      .adhc-seg.adhc-legendary::before { left:8%;  top:18%; width:32%; height:24%; opacity:.18; animation-delay:0s; }
      .adhc-seg.adhc-legendary::after  { left:42%; top:56%; width:24%; height:12%; opacity:.14; animation-delay:1.6s; }
      /* sweep: franja de luz que barre el tramo de izquierda a derecha */
      .adhc-seg.adhc-legendary .adhc-sweep {
        ...
        animation: adhc-sweep var(--sweep-dur,3.8s) ease-in-out infinite;
        animation-delay:var(--sweep-delay,0s);
      }
      /* flash: destello puntual que aparece y se desvanece */
      .adhc-seg.adhc-legendary .adhc-flash {
        position:absolute; left:30%; top:20%; width:7%; height:42%; border-radius:9999px; background:#fff;
        opacity:0; animation: adhc-flash var(--flash-dur,5.2s) ease-in-out infinite;
        animation-delay:var(--flash-delay,1.2s);
      }
      @keyframes adhc-shine-pulse { 0%,100%{opacity:.12} 50%{opacity:.3} }
      @keyframes adhc-sweep {
        0% { transform:translateX(-130%); opacity:0; }
        10% { opacity:.4; }
        40% { opacity:.4; }
        55%,100% { transform:translateX(240%); opacity:0; }
      }
      @keyframes adhc-flash { 0%,100%{opacity:0} 12%{opacity:.55} 30%,55%{opacity:.12} 75%{opacity:.35} }

      /* ===== separador/hito: píldora vertical con badge ===== */
      .adhc-sep { position:absolute; top:0; bottom:0; transform:translateX(-50%); z-index:4; border-radius:2px; transition:background .3s ease; }
      .adhc-sep-num { position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); min-width:16px; height:16px; line-height:16px; padding:0 2px; text-align:center; background:#fff; border-radius:9999px; font:700 10px/16px duolingo-sans,"Duolingo Sans",sans-serif; color:#1cb0f6; box-shadow:0 1px 2px rgba(0,0,0,.25); white-space:nowrap; z-index:5; }

      /* ===== partículas del burst (celebración de hito): 2s, grandes ===== */
      .adhc-part { position:fixed; width:14px; height:14px; border-radius:50%; pointer-events:none; z-index:4000; animation:adhc-burst 2s ease-out forwards; box-shadow:0 0 8px rgba(255,255,255,.6); }
      @keyframes adhc-burst { 0%{transform:translate(0,0) scale(1); opacity:1} 100%{transform:translate(var(--dx),var(--dy)) scale(.3); opacity:0} }

      /* ===== botón ⚙ (sombra dura Duolingo: 0 4px 0 mismo tono, sin blur) ===== */
      .adhc-btn { position:fixed; z-index:2000; right:18px; top:70px; background:#fff; border:none; border-radius:12px; width:36px; height:36px; display:flex; align-items:center; justify-content:center; cursor:pointer; font-size:16px; line-height:1; color:#1cb0f6; font-weight:700; box-shadow:0 4px 0 #e5e5e5; transition:transform .1s ease, box-shadow .1s ease; }
      .adhc-btn:hover { border-color:#1cb0f6; }
      .adhc-btn:active { transform:translateY(4px); box-shadow:0 0 0 #e5e5e5; }

      /* ===== panel config ===== */
      .adhc-panel { position:fixed; z-index:1000; background:#fff; border-radius:16px; box-shadow:0 4px 12px rgba(0,0,0,.15); padding:16px; font-family:duolingo-sans,"Duolingo Sans",sans-serif; min-width:260px; }
      .adhc-panel h4 { margin:0 0 12px; font-size:15px; color:#0a7ec2; } /* #1cb0f6 fallaba WCAG AA (2.9:1); #0a7ec2 = 5.6:1 */
      .adhc-panel label { display:block; font-size:13px; color:#4b4b4b; margin:10px 0 4px; }
      .adhc-panel input[type=range] { width:100%; }
      .adhc-panel .adhc-val { font-weight:700; color:#0a7ec2; }
      /* botones del panel: sombra dura 4px estilo Feather (mismo lenguaje que .adhc-btn) */
      .adhc-panel button { display:block; width:100%; margin-top:10px; padding:7px; background:#fff; border:2px solid #e5e5e5; border-radius:12px; font:700 13px duolingo-sans,"Duolingo Sans",sans-serif; color:#4b4b4b; cursor:pointer; box-shadow:0 4px 0 #d3d3d3; transition:transform .1s ease, box-shadow .1s ease; }
      .adhc-panel button:active { transform:translateY(4px); box-shadow:0 0 0 #d3d3d3; }
      .adhc-divider { margin-top:12px; border-top:1px solid #e5e5e5; padding-top:10px; }
    `);
    ...
    function ensureRoots() {
      ...
      overlay = bar.querySelector('.adhc-overlay');
      if (!overlay) {
        bar.dataset.adhcRooted = '1';
        overlay = document.createElement('div');
        overlay.className = 'adhc-overlay';
    ...
    function render() {
      ...
      overlay.querySelectorAll('.adhc-sep').forEach(sep => {
        ...
        if (pct + 1e-9 >= hitAt) {
          ...
          const badge = sep.querySelector('.adhc-sep-num');
          if (badge) {
    ...
    function injectSettingsBtn() {
      ...
      settingsBtn.className = 'adhc-btn';
    ...
    function togglePanel() {
      ...
      panel.className = 'adhc-panel';
      panel.innerHTML = `
        <h4>${tr(cfg.lang, 'panelTitle')}</h4>
        <label for="adhc-lang">${tr(cfg.lang, 'langLabel')}</label>
        <select id="adhc-lang">
          <option value="es" ${cfg.lang === 'es' ? 'selected' : ''}>Español</option>
          <option value="en" ${cfg.lang === 'en' ? 'selected' : ''}>English</option>
        </select>
        <label for="adhc-range">${tr(cfg.lang, 'labelMilestones')} <span class="adhc-val" id="adhc-pn">${cfg.separators}</span></label>
        <input type="range" id="adhc-range" min="1" max="12" value="${cfg.separators}" aria-label="${tr(cfg.lang, 'ariaMilestones')}">
        <label><input type="checkbox" id="adhc-num" ${cfg.showNumbers ? 'checked' : ''}> ${tr(cfg.lang, 'labelShowNumbers')}</label>
        <button id="adhc-reset">${tr(cfg.lang, 'btnReset')}</button>
      `;
      document.body.appendChild(panel);
      panel.style.right = '20px';
      panel.style.top = '80px';

      panel.querySelector('#adhc-lang').addEventListener('change', (e) => {
        cfg.lang = e.target.value;
        save();
        panel.remove(); panel = null;
        togglePanel(); // reconstruye con el nuevo idioma
      });
      const range = panel.querySelector('#adhc-range');
      range.addEventListener('input', () => { panel.querySelector('#adhc-pn').textContent = range.value; });
      const done = () => {
        cfg.separators = parseInt(range.value, 10);
        cfg.showNumbers = panel.querySelector('#adhc-num').checked;
        save();
        buildOverlayContent(); render();
      };
      range.addEventListener('change', done);
      panel.querySelector('#adhc-num').addEventListener('change', done);
      panel.querySelector('#adhc-reset').addEventListener('click', () => {
        const keepLang = cfg.lang;
        Object.assign(cfg, DEFAULTS, { lang: keepLang });
        save();
        range.value = cfg.separators; panel.querySelector('#adhc-pn').textContent = cfg.separators;
        panel.querySelector('#adhc-num').checked = cfg.showNumbers;
        buildOverlayContent(); render();
      });
      setTimeout(() => {
